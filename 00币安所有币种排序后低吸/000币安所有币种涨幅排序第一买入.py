...
def K_line(a):  #获取K线数据
    ...
    try:
        ...
        if (ttt[-1][0]/1000)//bbb[ddd["interval"]]==(time.time()-ab_time/1000)//bbb[ddd["interval"]]-1:
            ...
            lock.acquire()
            eee[a]={"openn":float(ttt[-1][1]), "closee":float(ttt[-1][4])}
            #eee.append([float(ttt[-1][4])/float(ttt[-1][1]), a])
            lock.release()
        else:
            print(f"{a}: 2不正常====")
...
Secret_Key="tTF2jRlWT9o1i0wJGWLlwjbCL4O8RHTdiIuxLxdEU7T5oj1PzNuOvrOiS7FkrfWC"
API_Key="WlXxYFOuTLqYg61A5scG1ZKLjQNvOPNS0tzrdOgaiWRa349uop8GtpwLpeqGo5bL"
    ...
    while True:
        ...
        try:
            ...
            if run==True:
                ...
                for ee in eee.keys():
                    if float(ddd["symbols"][ee]["balances"])*float(eee[ee]["closee"])>20:  #卖出
                        ddd["symbols"][ee]["kline_result"]=-1  #卖出
...
##                #查询账户余额==============
                ...
                fff=[]
                for ee in eee.keys():
                    fff.append([eee[ee]["closee"]/eee[ee]["openn"], ee])
                ...
                print(fff_numm/fff_len)
                
                if float(ddd["symbols"][fff[0][1]]["balances"])*float(eee[fff[0][1]]["closee"])<=20 and fff_numm/fff_len>0.6:  #计算是否买入
                    ddd["symbols"][fff[0][1]]["kline_result"]=1  #买入
                    ddd["symbols"][fff[0][1]]["buy_price"]=eee[fff[0][1]]["closee"]
                    ddd["symbols"][fff[0][1]]["sub_price"]=abs(eee[fff[0][1]]["closee"]-eee[fff[0][1]]["openn"])

##                if float(ddd["symbols"][fff[1][1]]["balances"])*float(eee[fff[1][1]]["closee"])<=20:  #计算是否买入
##                    ddd["symbols"][fff[1][1]]["kline_result"]=1  #买入
##                    ddd["symbols"][fff[1][1]]["buy_price"]=eee[fff[1][1]]["closee"]
##                    ddd["symbols"][fff[1][1]]["sub_price"]=abs(eee[fff[1][1]]["closee"]-eee[fff[1][1]]["openn"])
##
##                if float(ddd["symbols"][fff[2][1]]["balances"])*float(eee[fff[2][1]]["closee"])<=20:  #计算是否买入
##                    ddd["symbols"][fff[2][1]]["kline_result"]=1  #买入
##                    ddd["symbols"][fff[2][1]]["buy_price"]=eee[fff[2][1]]["closee"]
##                    ddd["symbols"][fff[2][1]]["sub_price"]=abs(eee[fff[2][1]]["closee"]-eee[fff[2][1]]["openn"])
