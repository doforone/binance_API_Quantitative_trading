        ...
        if (ttt[-1][0]/1000)//bbb[ddd["interval"]]==(time.time()-ab_time/1000)//bbb[ddd["interval"]]-1:
            ...
            if float(ddd["symbols"][a]["balances"])*float(ttt[-1][4])<=20:  #计算是否买入
                if ddd["symbols"][a]["buy_price"]==-1:
                    ...
                    lock.release()
                elif mmm_nnn[-1]<-max(mmm_nnn[:-1])*ddd["AB"] and mmm_nnn[-1]<min(mmm_nnn[:-1])*ddd["AB"] and mmm_nnn[-2]<0:
                    lock.acquire()
                    ddd["symbols"][a]["kline_result"]=1  #买入
...
##                   and float(ddd["symbols"][a]["balances"])*float(ttt[-1][4])>20:
                if (float(ttt[-1][4])>ddd["symbols"][a]["buy_price"]+ddd["AA"]*ddd["symbols"][a]["sub_price"]) or \
...
if __name__ == "__main__":
    while True:
        ...
        try:
            ...
            #读取all_set文件-------
            with open('all_set.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
            ...
            if ttt["all_set"]==-1:
                ...
                if curr_timee%bbb[ddd["interval"]]>=10\
...
##                    #==================================
                ...
                print(str(run_num)+"  币安000低吸高抛_4版")
