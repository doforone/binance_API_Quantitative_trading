        ...
        if ttt["status"]=="ok":
            ...
            if ttt["data"][-1]["id"]//bbb[ddd["interval"]]==timee_ab()//bbb[
                ...
                if float(ddd["symbols"][a]["balances"])<ddd["symbols"][a][
                    ...
                    if ddd["symbols"][a]["buy_price"]==-1:
                        lock.acquire()
                        ddd["symbols"][a]["kline_result"]=1  #买入
                        ddd["symbols"][a]["buy_price"]=999999
                        ddd["symbols"][a]["sub_price"]=111111
                        lock.release()
                    elif mmm_nnn[-1]<-max(mmm_nnn[:-1])*ddd["AB"]\
...
##                               "VV"]*ddd["symbols"][a]["sub_price"]:
                    ...
                    if (ttt["data"][-1]["close"]>ddd["symbols"][a]["buy_price"]+ddd["AA"]*ddd["symbols"][a]["sub_price"]) or \
                       (mmm_nnn[-1]>max(mmm_nnn[:-1])*ddd["AB"] and mmm_nnn[-1]>-min(mmm_nnn[:-1])*ddd["AB"]):
                        lock.acquire()
...
if __name__ == "__main__":
    while True:
        ...
        try:
            #读取all_set文件-------
            with open('all_set.txt', 'r', encoding='utf-8-sig',
                      newline='\r\n') as f:
            ...
            if ttt["all_set"]==-1:
                ...
                if curr_timee%bbb[ddd["interval"]]>=6\
                   and curr_timee%bbb[ddd["interval"]]<120\
                   and curr_timee//bbb[ddd["interval"]]>last_time//bbb[ddd["interval"]]:  #>=6是为了时间的保守，因为有些小币种K线生成太慢
                    run=True
            #校对时间并确定间隔区间=============
...
##                #查询账户余额==============
                ...
                have=1
                numm=0
                #while have==1 and ttt["all_set"]==0 and numm<4:  #全部卖出的时候，就不再买入了
                while have==1 and numm<4:  #全部卖出的时候，就不再买入了
                    have=0
                    for symbol in ddd["symbols"].keys():
                        if ddd["symbols"][symbol]["kline_result"]==1\
...
##                    #==================================
                ...
                if ttt["all_set"]==1:
                    ttt["all_set"]=0
                    with open('all_set.txt', 'w', encoding='utf-8', newline='\r\n') as f:
                ...
                print(str(run_num)+"  火币000低吸高抛_4版")
